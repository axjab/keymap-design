...
This principle is not absolute. The Silakka54's reduced key count necessarily requires remapping and layering. "Conservation" therefore means preserving relationships when practical and when doing so does not conflict with higher-priority design considerations.
...
A provisional classification is:
...
normal text production, and
correction of errors made during text production.
...
Leftmost/right-cluster large key	1.75u	Tab	Space	Tab represents grouped whitespace
...
Mnemonic: End the line/statement.
...
Topology Conservation — Space remains adjacent to and left of Enter.
Priority — all three are high-value editing operations.
Functional Grouping — editing operations are colocated.
Frequency — the largest/easiest position is assigned to Space, while Backspace remains highly accessible despite its low desired steady-state frequency.
Physical constraints — the 1.75u key accepts an appropriate donor keycap while the conventional Spacebar does not.
Semantic mnemonic quality — Tab/Space, Delete/Backspace, and End/Enter each have defensible associations.
7. Modifier Strategy
...
The right cluster is dedicated primarily to:
